...
const ManageClasses = () => {
  ...
  const handleAddClass = async () => {
    try {
      ...
      // Insert a new class using helper function
  ...
  const handleEditClass = async () => {
    try {
      ...
      // Check if another class with same name and section already exists (excluding current class)
      const { data: existingClass } = await supabase
        .from(TABLES.CLASSES)
        .select('id')
      ...
      // Update a class using helper function
      const { data, error } = await dbHelpers.updateClass(selectedClass.id, {
          ...
          onPress: async () => {
            try {
              ...
              if (studentsInClass && studentsInClass.length > 0) {
                Alert.alert(
                  'Cannot Delete Class',
                  `This class has ${studentsInClass.length} student(s) assigned to it. Please reassign or remove the students first before deleting the class.`,
                  [{ text: 'OK' }]
                );
                return;
              }

              // 1. Delete all subjects for this class
              ...
              // 2. Delete timetable entries for this class
              const { error: timetableError } = await supabase
                .from(TABLES.TIMETABLE_ENTRIES)
                .delete()
                .eq('class_id', classId);
              if (timetableError) console.warn('Error deleting timetable entries:', timetableError);

              // 3. Delete the class using helper function
              const { error: classDeleteError } = await dbHelpers.deleteClass(classId);
